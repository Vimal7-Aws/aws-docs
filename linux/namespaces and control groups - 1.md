...
> All the code for this article is available in this [GitHub repo](https://github.com/your-repo-link-here).
...
First, make sure that your Linux kernel supports user namespaces. To do so, find the configuration file for your kernel and use grep to search for `CONFIG_USER_NS`. The following command is an example of the Ubuntu kernel:
...
> ```bash
> cat /sys/fs/cgroup/system.slice/docker-your_container_id.scope/cpu.max
> ```
...
> ```bash
> cat /sys/fs/cgroup/system.slice/docker-your_new_container_id.scope/cpu.max
> ```
...
If you want to run a Docker container with memory limits using cgroups, you can use the `--memory` option with the `docker run` command. The following example uses the official Nginx image from Docker Hub:
...
As the landscape of containerization continues to evolve, a solid grasp of Linux namespaces and cgroups empowers users to harness the full potential of Docker and deliver high-performance, scalable applications. In this article, you learned how namespaces and cgroups are
